...
export function createDioramaFacility(structureId: string): THREE.Group {
  ...
  function box(size: Point, position: Point, color: Color, radius = 0) {
    const geometry = radius
      ? new RoundedBoxGeometry(...size, 2, radius)
      : cube.clone().scale(...size);
  ...
  switch (structureId) {
    ...
    case "retention-basin": {
      ...
      box([10, 1.4, 11], [37, 12.6, -2], "blue", 0.5);
      break;
    }
    case "drainage-pump": {
  ...
  for (const [color, parts] of batches) {
    ...
    const material = new THREE.MeshStandardMaterial({
      color: COLORS[color], roughness: color === "water" ? 0.3 : 0.72, metalness: 0,
    });
