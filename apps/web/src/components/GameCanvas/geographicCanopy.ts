...
import {
  imageryPixelToGeo,
  type ImageryTreeCandidate,
  type ImageryTreeObservations,
  type VegetationPoint,
  type VegetationBounds,
  type VegetationExclusion,
} from "./imageryVegetation";
import {
  distanceToVegetationSegment,
  insideVegetationRing,
  intersectsVegetationExclusion,
} from "./geographicImageryVegetation";
import {
  createVegetationStyleResources,
  vegetationColor,
  vegetationHashUnit,
  VEGETATION_STYLE_PROVENANCE,
} from "./geographicVegetationStyle";

export const IMAGERY_CANOPY_URL = "/geodata/koriyama/imagery-canopy-observations.json";
export const GAME_CANOPY_DENSITY = {
  spacingM: 3.2,
  radiusRangeM: [1.1, 1.65] as const,
  maxTrees: 2000,
};
type IllustrativeCanopyProfile = "riverbank-detail-v1";
const RIVERBANK_DETAIL_IDS = new Set([
...
function validateProfile(patch: { id: string; illustrativeProfile?: IllustrativeCanopyProfile }) {
  if (
    patch.illustrativeProfile !== undefined &&
    (patch.illustrativeProfile !== "riverbank-detail-v1" || !RIVERBANK_DETAIL_IDS.has(patch.id))
  )
    throw new RangeError("Invalid/out-of-scope canopy profile");
}
...
export function convertImageryCanopyObservations(data: ImageryCanopyObservations): CanopyPatch[] {
  ...
  if (
    ...
    !/^\d{4}-\d{2}\/\d{4}-\d{2}$/.test(data.source.displayedCapturePeriod)
  )
    throw new RangeError("Invalid canopy source metadata");
  imageryPixelToGeo(ref, { x: 0, y: 0 });
  data.patches.forEach(validateProfile);
  return data.patches.map((patch) => ({
    id: patch.id,
    source: data.source,
    observationView: patch.observationView,
    illustrativeProfile: patch.illustrativeProfile,
    rings: patch.rings.map((ring) =>
      ring.map(([east, south]) => {
        if (![east, south].every(Number.isFinite)) throw new RangeError("Invalid canopy pixel");
        const dx = Math.floor(east / 256),
          dy = Math.floor(south / 256);
        return koriyamaGeoToLocal(
          imageryPixelToGeo(
            { z: ref.z, x: ref.x + dx, y: ref.y + dy },
            { x: east - dx * 256, y: south - dy * 256 },
          ),
        );
      }),
    ),
...
export function canopyContainsCrown(
  rings: readonly (readonly VegetationPoint[])[],
  p: VegetationPoint,
  radius: number,
) {
  if (
    ...
    radius <= 0 ||
    !rings[0]?.length ||
    !insideVegetationRing(p, rings[0]) ||
    rings.slice(1).some((r) => insideVegetationRing(p, r))
  )
    return false;
  for (const ring of rings)
    for (let i = 0; i < ring.length; i++) {
      if (distanceToVegetationSegment(p, ring[i]!, ring[(i + 1) % ring.length]!) <= radius + 1e-6)
        return false;
    }
  return true;
}
  ...
  for (const ring of rings)
    for (const p of ring) {
      if (!Number.isFinite(p.x) || !Number.isFinite(p.z))
        throw new RangeError("Invalid canopy/exclusion coordinate");
...
export type CanopyRecord = {
  ...
  reason: Reason;
  exclusionIds: string[];
  positionSource: "illustrative-within-imagery-envelope";
};
...
export function createGeographicCanopy(patches: readonly CanopyPatch[], options: CanopyOptions) {
  const { bounds: b } = options,
    spacing = options.spacingM ?? 6;
  const radii = options.radiusRangeM ?? [2.3, 3.5],
    heights = options.heightRangeM ?? [6, 9];
  const maxCells = options.maxCells ?? 8192,
    maxTrees = options.maxTrees ?? 1500;
  const tileSize = options.tileSize ?? 256,
    batchSize = options.maxInstancesPerBatch ?? 512;
  if (
    ...
    maxCells < 1 ||
    maxCells > 20000 ||
    !Number.isInteger(maxTrees) ||
    maxTrees < 0 ||
    maxTrees > 5000 ||
    !Number.isInteger(batchSize) ||
    batchSize < 1 ||
    batchSize > 4096 ||
    ![...radii, ...heights].every((v) => Number.isFinite(v) && v > 0 && v <= 30) ||
    radii[0] > radii[1] ||
    heights[0] > heights[1]
  )
    throw new RangeError("Invalid canopy limits");
  ...
  for (const patch of patches) {
    ...
    if (
      ...
      seen.has(patch.id) ||
      !patch.rings.length ||
      patch.rings.some((r) => r.length < 3 || r.length > 512)
    )
      throw new RangeError("Invalid/duplicate canopy patch");
    boundsOf(patch.rings);
    seen.add(patch.id);
  }
  const indexed = options.exclusions.map((e) => {
    const g = e.geometry,
      bb = boundsOf(
        g.type === "polygon" ? g.rings : g.type === "corridor" ? [g.points] : [[g.centre]],
      );
    const pad = g.type === "corridor" ? g.halfWidthM : g.type === "disc" ? g.radiusM : 0;
    if (!Number.isFinite(pad) || pad < 0 || !Object.values(bb).every(Number.isFinite))
      throw new RangeError("Invalid canopy exclusion");
    return {
      e,
      bounds: {
        minX: bb.minX - pad,
  ...
  const explicit = options.observedCrowns.map((c) => {
    ...
    if (![p.x, p.z, c.crownRadiusM].every(Number.isFinite) || c.crownRadiusM <= 0)
      throw new RangeError("Invalid explicit crown exclusion");
    return { p, radius: c.crownRadiusM, id: c.id };
  });
  const records: CanopyRecord[] = [],
    accepted: CanopyRecord[] = [];
  ...
  for (const patch of [...patches].sort((a, c) => (a.id < c.id ? -1 : a.id > c.id ? 1 : 0))) {
    const profile =
      patch.illustrativeProfile === "riverbank-detail-v1" ? RIVERBANK_DETAIL : undefined;
    ...
    if (!overlaps(pb, b)) {
      skippedPatches.push({ id: patch.id, reason: "outside-bounds" });
      continue;
    }
    const x0 = Math.floor(Math.max(pb.minX, b.minX) / patchSpacing),
      x1 = Math.ceil(Math.min(pb.maxX, b.maxX) / patchSpacing);
    const z0 = Math.floor(Math.max(pb.minZ, b.minZ) / patchSpacing),
      z1 = Math.ceil(Math.min(pb.maxZ, b.maxZ) / patchSpacing);
    const cells = (x1 - x0) * (z1 - z0);
    if (!Number.isSafeInteger(cells) || cells > maxCells - evaluatedCells) {
      skippedPatches.push({ id: patch.id, reason: "cell-budget" });
      continue;
    }
    ...
    for (let iz = z0; iz < z1; iz++)
      for (let ix = x0; ix < x1; ix++) {
        ...
        const x = (ix + 0.5 + (vegetationHashUnit(`${id}/x`) - 0.5) * 0.3) * patchSpacing;
        const z = (iz + 0.5 + (vegetationHashUnit(`${id}/z`) - 0.5) * 0.3) * patchSpacing;
        const radius =
          patchRadii[0] + (patchRadii[1] - patchRadii[0]) * vegetationHashUnit(`${id}/radius`);
        const height = heights[0] + (heights[1] - heights[0]) * vegetationHashUnit(`${id}/height`);
        const r: CanopyRecord = {
          id,
          patchId: patch.id,
          x,
          z,
        ...
        if (
          ...
          z - radius < b.minZ ||
          z + radius > b.maxZ ||
          !canopyContainsCrown(patch.rings, r, radius)
        ) {
          r.reason = "boundary";
          continue;
        }
        for (const c of explicit)
          if (Math.hypot(x - c.p.x, z - c.p.z) <= radius + c.radius) r.exclusionIds.push(c.id);
        if (r.exclusionIds.length) {
          r.reason = "explicit-crown";
          continue;
        }
        const crownBounds = {
          minX: x - radius,
        ...
        for (const entry of obstacles)
          if (
            overlaps(crownBounds, entry.bounds) &&
            intersectsVegetationExclusion(r, radius, entry.e)
          )
            r.exclusionIds.push(entry.e.sourceId);
        if (r.exclusionIds.length) {
          r.reason = "excluded";
          continue;
        }
        if (accepted.some((a) => Math.hypot(x - a.x, z - a.z) <= radius + a.radiusM)) {
          r.reason = "reconstruction-overlap";
          continue;
        }
        if (accepted.length >= maxTrees) {
          r.reason = "capacity";
          continue;
        }
        ...
        for (let i = 0; i < 8 && valid; i++) {
          const y = options.groundSampler(
            x + radius * Math.cos((i * Math.PI) / 4),
            z + radius * Math.sin((i * Math.PI) / 4),
          );
          if (y === null || !Number.isFinite(y)) valid = false;
        }
        if (!valid) {
          r.reason = "no-ground-data";
          continue;
        }
        r.y = base!;
        accepted.push(r);
  ...
  group.name = "imagery-envelope-canopy-reconstruction";
      ...
      for (let start = 0; start < trees.length; start += batchSize) {
        ...
        for (const part of ["trunk", "crown"] as const) {
          ...
          mesh.userData = {
            part,
            tile: key,
            reconstructionIds: chunk.map((r) => r.id),
            positionSource: "illustrative-within-imagery-envelope",
          };
          chunk.forEach((r, i) => {
            dummy.position.set(r.x, r.y! + r.heightM * (part === "trunk" ? 0.25 : 0.65), r.z);
            const horizontalScale = part === "trunk" ? Math.min(1, r.radiusM / 0.2) : r.radiusM;
            dummy.scale.set(
              horizontalScale,
              r.heightM * (part === "trunk" ? 0.5 : 0.35),
              horizontalScale,
            );
            dummy.updateMatrix();
  ...
  group.userData = {
    ...
    stats,
    patches,
    style: VEGETATION_STYLE_PROVENANCE,
    positionSource: "illustrative-within-imagery-envelope",
    dimensionsSource: "illustrative-not-measured",
    policy: {
      spacingM: spacing,
      radiusRangeM: radii,
      heightRangeM: heights,
      maxCells,
      maxTrees,
      seed: "patch-id-world-grid-fnv1a-v1",
      patchProfiles: patches
        .filter((p) => p.illustrativeProfile)
        .map((p) => ({
          id: p.id,
          profile: p.illustrativeProfile,
          spacingM: options.spacingM ?? RIVERBANK_DETAIL.spacingM,
          radiusRangeM: options.radiusRangeM ?? RIVERBANK_DETAIL.radiusRangeM,
        })),
    },
    attributions: [...new Set(patches.map((p) => p.source.attribution))],
    limitations:
      "Only envelopes observed in imagery; internal centres/counts/density/radii/heights reconstructed, not individual observations. Capture period is a view label, not verified per-patch acquisition metadata. No placement beyond supplied envelopes.",
  };
