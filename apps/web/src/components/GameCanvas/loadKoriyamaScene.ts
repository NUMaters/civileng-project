...
import { KORIYAMA_LANDCOVER_URL, type KoriyamaLandcover } from "./koriyamaLandcover";
import { IMAGERY_TREE_OBSERVATIONS_URL, convertImageryTreeObservations, type ImageryTreeObservations } from "./imageryVegetation";
import { IMAGERY_CANOPY_URL, convertImageryCanopyObservations, type ImageryCanopyObservations } from "./geographicCanopy";
...
export async function loadKoriyamaScene(signal: AbortSignal) {
  ...
  const [osm, plateau, metadata, binary, landcover, imageryObservations, canopyObservations] = await Promise.all([
    ...
    read(KORIYAMA_LANDCOVER_URL).then(r => r.json() as Promise<KoriyamaLandcover>),
    read(IMAGERY_TREE_OBSERVATIONS_URL).then(r => r.json() as Promise<ImageryTreeObservations>),
    read(IMAGERY_CANOPY_URL).then(r => r.json() as Promise<ImageryCanopyObservations>),
  ]);
  signal.throwIfAborted();
  for (const collection of [osm, plateau, landcover]) {
  ...
  return { osm, plateau, landcover, imageryTrees: convertImageryTreeObservations(imageryObservations),
    canopyPatches: convertImageryCanopyObservations(canopyObservations), terrain: decodeKoriyamaTerrain(binary, metadata) };
}
