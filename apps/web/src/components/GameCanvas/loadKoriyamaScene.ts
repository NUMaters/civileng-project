...
export async function loadKoriyamaScene(signal: AbortSignal) {
  ...
  const [osm, plateau, metadata, binary, landcover, imageryObservations] = await Promise.all([
  ...
  return { osm, plateau, landcover, imageryTrees: convertImageryTreeObservations(imageryObservations), terrain: decodeKoriyamaTerrain(binary, metadata) };
}
