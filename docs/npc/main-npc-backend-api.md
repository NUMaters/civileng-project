...
NPC Backend
  ├─ NPC・質問・Factの対応を再検証
  ├─ 承認済みFactだけでLLMを実行し、NPCのペルソナに合う文章を生成
...
現在のMain Backendにはサーバー権威のフェーズ・災害状態がまだないため、ローカルMVPでは公開会話APIが受け取った状態を検証する橋渡し実装を使用する。ゲーム状態管理の完成後、この橋渡しだけをサーバー権威の判定へ置き換える。内部API契約は変更しない。この橋渡しは本番の認可実装ではない。
...
失敗時も、入力契約が正しければHTTP 200で論理結果を返す。Main Backendは`answerText`を信用せず、相関ID・表示可能な会話状態・期待する`sourceIds`を再検証する。
...
準備フェーズの成人向け回答では、NPC Backendが該当するNPCペルソナ、選択質問、ヒント段階、承認済みFactだけをOpenAIへ渡す。モデルは回答本文だけをJSONで返す。

- モデルは出典IDを選ばない。`sourceIds`はNPC BackendがFactから確定する。
- 生成文は120文字以内、最大2文、URL・HTML・安全性を断定する表現を含まないことを検証する。
- 指定Factの取得失敗、タイムアウト、JSON形式不正、安全性検証失敗では`success`にせず、Main Backend側で承認済み固定回答を表示する。
- 根拠の意味的な正しさは、承認済みFactのみをプロンプトへ渡すことと、回答テストで担保する。モデルが新しい資料やゲーム外の事実を参照する経路は持たない。
