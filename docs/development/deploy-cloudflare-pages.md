...
## 2. ダッシュボードから接続（現在は使用しない）

このリポジトリは GitHub Actions から Wrangler API で Pages にデプロイするため、Cloudflare の Git 連携は使用しない。Cloudflare Workers の Git 連携を同じリポジトリへ接続すると、PR ごとに Workers Builds が起動し、Pages のビルド設定と衝突しやすい。既存の `civileng-project` Workers Builds 接続は切断済みである。

新しい Pages プロジェクトを Git 連携で作成する場合のみ、以下を参照すること。既存プロジェクトの自動デプロイを有効化する場合は「4. GitHub Actions 自動デプロイ」を使う。
...
| `Authentication error [code: 10000]` / `Invalid access token [code: 9109]` | GitHub Secret の API トークン期限切れ、またはトークン種別と検証エンドポイントの不一致。アカウント所有トークンは account API の検証エンドポイントを使い、Cloudflare Dashboard で Pages 編集権限付きトークンを再発行して `CLOUDFLARE_API_TOKEN` を更新する。ローカルは `wrangler login` し直して `pnpm deploy:web` |
