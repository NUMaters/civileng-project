...
最大参加人数は4人とする。上限は `MULTIPLAYER_MAX_PLAYERS = 4` として管理し、仕様変更時にコードへ散在させない。
...
ルーム作成者が「開始」を押した時点でゲーム開始要求を送信する。作成者が開始操作を行わない場合は、ロビー作成から30秒経過した時点で自動的に開始要求を送信する。
...
ready待ち時間はゲーム開始要求から **30秒** とする。開始要求からの待ち時間は `SESSION_READY_TIMEOUT_SECONDS = 30` として管理する。
...
マルチプレイヤーの初期予算は1プレイヤーあたり **8,500pt** とする。値は `MULTIPLAYER_INITIAL_BUDGET_PT = 8500` としてゲームデータのマスターデータで管理し、画面やサーバーの処理へ直接ハードコードしない。
...
他プレイヤーのマーカーを表示する。自分自身の現在位置は、画面上の既存のカメラ・操作状態で把握できるため、追加マーカーは表示しない。
...
送信頻度は最大 **5回/秒** とする。上限は `PLAYER_MOVE_MAX_HZ = 5` として管理する。

最短送信間隔は200msとする。間隔は `PLAYER_MOVE_MIN_INTERVAL_MS = 200` として管理する。
...
ゲーム進行中の権威状態はGame Serverのメモリに保持する。Alpha-M2ではGame Serverを単一プロセスとして運用するため、Redisなどの外部共有ストアは使用しない。将来、Game Serverを水平スケールする場合は、Sessionの所有権とイベント配信を外部ストアへ移す設計を別途定義する。
